...
static ssize_t _riot_realtime_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len, coap_request_ctx_t *context)
{
    (void)context;
    ssize_t p = 0;
    char rsp[16];
    ...
    len = snprintf((char *)buf, sizeof(buf), "%" PRIi64, (int64_t)_now);
    
    /* write the response buffer with the internal value */
    p += fmt_u32_dec(rsp, *buf);
...
NANOCOAP_RESOURCE(realtime) {
    .path = "/riot/realtime", .methods = COAP_GET, .handler = _riot_realtime_handler
};
