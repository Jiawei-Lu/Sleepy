...
int main(void)
{
    ...
    while (1){
        ...
        if (ds3231_get_time(&_dev, &sync_time) != 0) {
        ...
        if (ds3231_set_alarm_1(&_dev, &sync_time, DS3231_AL1_TRIG_M_S) != 0) {
            puts("Error: alarm set failed");
            return 1;
        }
        ds3231_clear_alarm_1_flag(&_dev);
        pm_set(SAML21_PM_MODE_STANDBY);
