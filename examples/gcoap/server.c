...
static ssize_t _time_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx);

/* CoAP resources. Must be sorted by path (ASCII order). */
static const coap_resource_t _resources[] = {
    { "/cli/stats", COAP_GET | COAP_PUT, _stats_handler, NULL },
    { "/riot/board", COAP_GET, _riot_board_handler, NULL },
    { "/realtime", COAP_GET, _time_handler, NULL },
};
...
static ssize_t _stats_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx)
{
    ...
    switch (method_flag) {
        case COAP_GET:
            ...
            char test [12]= "1717510049";
            // req_count = (uint16_t)strtoul(test, NULL, 12);
            resp_len += fmt_str((char *)pdu->payload, test);
            return resp_len;
...
static ssize_t _time_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx)
{
    ...
    switch (method_flag) {
        case COAP_GET:
            ...
            /* write the response buffer with the system timestamp value value */
            time_t now = time(NULL);
            gmtime(&now);
            uint32_t req_count = now;

            resp_len += fmt_u32_dec((char *)pdu->payload, req_count);
            return resp_len;
    }
    return 0;
}
