...
static ssize_t _sleepy_data_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx);

/* CoAP resources. Must be sorted by path (ASCII order). */
static const coap_resource_t _resources[] = {
    { "/cli/stats", COAP_GET | COAP_PUT, _stats_handler, NULL },
    { "/riot/board", COAP_GET, _riot_board_handler, NULL },
    { "sleepy/data", COAP_GET, _sleepy_data_handler, NULL},
};
...
static ssize_t _sleepy_data_handler(coap_pkt_t *pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx)
{
    ...
    if (pdu->payload_len >= strlen(com_buffer)) {
        memcpy(pdu->payload, com_buffer, strlen(com_buffer));
        return resp_len + strlen(com_buffer);
    }
    else {
